...
class _MineViewState extends State<MineView> {
      ...
      child: Row(
        children: [
          Obx(() {
            return CircleAvatar(
              radius: 26,
              backgroundImage: _userController.users.value.avatar.isNotEmpty
                  ? NetworkImage(_userController.users.value.avatar)
              ...
              children: [
                Obx(() {
                  return GestureDetector(
                    onTap: () {
                      if (_userController.users.value.id.isEmpty) {
                        Navigator.pushNamed(context, '/login');
                      }
                    },
                    child: Text(
                      _userController.users.value.id.isNotEmpty
                          ? _userController.users.value.account
                          : '立即登录',
                      style: TextStyle(
                        fontSize: 18,
  ...
  void _getGuessLikeList() async {
    if (_isLoading) return;
    _isLoading = true;
    final res = await getGuessLikeListAPI(_params);
    _list.addAll(res.items);
    hasMore = _params['page'] < res.pages;
    setState(() {
      if (hasMore) _params['page']++;
      _isLoading = false;
    });
  }
